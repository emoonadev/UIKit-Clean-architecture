...
public protocol ObservableSearchBar {
    var textDidChange: Observable<String> { get }
    var text: String? { get set }
}

public class TableViewDataSourceProvider<Item, Cell: UITableViewCell & NibLoadableView>: NSObject, UITableViewDataSource, UITableViewDelegate, UISearchResultsUpdating {
    public typealias CellConfigurator = (_ indexPath: IndexPath, _ item: Item, _ cell: Cell) -> Void
    public typealias HeightConfigurator = (_ indexPath: IndexPath, _ item: Item) -> CGFloat
    public typealias ContextMenuConfigurator = (_ indexPath: IndexPath, _ item: Item) -> UIMenu
    public typealias SwipeConfigurator = (_ rowAction: UIContextualAction, _ item: Item) -> Void
    public typealias SimpleConfigurator = (_ items: [Item], _ filteredItems: [Item], _ indexPath: IndexPath, _ item: Item) -> Void
    ...
    public init(items: [Item], cell: Cell.Type, isAutomaticDimension: Bool = true, cellConfigurator: @escaping CellConfigurator) {
        self.items = items
        self.cell = cell
    ...
    public init(items: Observable<[Item]>, cell: Cell.Type, isAutomaticDimension: Bool = true, cellConfigurator: @escaping CellConfigurator) {
...
public extension TableViewDataSourceProvider {
